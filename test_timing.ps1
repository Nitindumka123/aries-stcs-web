...
foreach ($u in @("/app/control","/app/observations","/app/syscam","/app/admin","/api/telemetry/snapshot")) {
  $sw = [Diagnostics.Stopwatch]::StartNew()
  try {
    $r = Invoke-WebRequest -Uri ("http://127.0.0.1:8000" + $u) -WebSession $s -UseBasicParsing -MaximumRedirection 2 -ErrorAction Stop
    $sw.Stop()
    Write-Output ("TIMED " + $u + " : " + $r.StatusCode + " in " + $sw.ElapsedMilliseconds + "ms")
  } catch {
    $sw.Stop()
    Write-Output ("TIMED " + $u + " FAILED in " + $sw.ElapsedMilliseconds + "ms : " + $_.Exception.Message)
  }
}
Stop-Job $job; Remove-Job $job
Write-Output "TIMING_DONE"
